...
# other function to optimize
def minimize_sum_of_absolute_differences(K, N, A, constraint_b, bounds):
    """
    Minimiert die Summe der absoluten Differenzen zwischen bestimmten Einträgen der Matrix A und 1/K
    unter der Bedingung Ax <= b.

    Parameters:
    K (int): Der Konstantenwert K.
    N (int): Die Anzahl der zu berücksichtigenden Einträge.
    A (ndarray): Die Eingabematrix.
    constraint_b (ndarray): Die rechte Seite der Ungleichungen.
    bounds (sequence): Schranken für die Variablen x.

    Returns:
    result (OptimizeResult): Das Ergebnis der Optimierung.
    """
    ...
    def objective(x):
        res = 0
        for i in range(N):
            res += abs(np.dot(A[2*i], x) - 1/K)
        return res 
    
    # Umwandlung der Ungleichungen in eine Funktion für 'minimize'
    def constraint_func(x):
        return constraint_b - np.dot(A, x)
    
    # Definition der Nebenbedingungen für 'minimize'
    cons = {'type': 'ineq', 'fun': constraint_func}

    # Startwert für die Optimierung
    x0 = [0] * len(A[0])  # Startwert ist die abgeflachte Version der Matrix A
...
# Beispielhafte Matrix A und Werte für K und N
A = np.array([
    [0.1, 0.2, 0.3, 0.4, 0.1, 0.2],
    [0.5, 0.6, 0.7, 0.8, 0.2, 0.2],
    [0.9, 1.0, 1.1, 1.2, 0.2, 0.2],
    [1.3, 1.4, 1.5, 1.6, 0.2, 0.3]
])

K = 3
N = 1

# Beispielhafte Werte für die Nebenbedingungen
constraint_b = np.array([0.5, 0.5, 0.5, 0.5])

# Schranken für die Variablen x
bounds = [(0, 1) for _ in range(6)]  # Beispielhafte Schranken, hier zwischen 0 und 1 für alle Variablen

# Aufruf der Minimierungsfunktion
result = minimize_sum_of_absolute_differences(K, N, A, constraint_b, bounds)
...
file_path_q = 'C:\\Users\\carol\\Downloads\\ex_q.txt'
file_path_p = 'C:\\Users\\carol\\Downloads\\ex_p.txt'
...
def algorithm_min(N, A, b_vek,p_alle, K):
    '''
    ...
    '''
    res_ok = 10
    value_range = [i * 0.1 for i in range(0,3)]
    rep =  K-1
    ...
    for combi in combinations:
        ...
        print(A, b_vek, x_bounds)
        result =  minimize_sum_of_absolute_differences(K, N, A, b_vek, x_bounds)
        resi = []
        for i in result.x:
            resi.append(i)
...
def repeat_algo(q_vectors, p_alle):
    ...
    N = len(q_vectors[0])
    res = algorithm_min(N, A, b_vek, p_alle,K)
    res_a.append(res)
    print("res_a", res_a)
    return res_a
...
def repeat_create_daten(q_alle, p_alle, K, parameters):
    ...
    speichere_werte(q, "population_q1_1_K3_max")
    speichere_werte(p, "population_p1_1_K3_max")
    return
#%%
